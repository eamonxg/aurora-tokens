...
const aOf = (v) => new Color(toOklch(v)).alpha;
...
test("set: fixes L and C, keeps hue", () => {
  const c = new Color(toOklch(set("oklch(0.5 0.1 200)", 0.9, 0.05))).to(
    "oklch",
  );
...
test("alpha: sets opacity", () => {
  assert.ok(Math.abs(aOf(alpha("oklch(0.6 0.1 200)", 0.5)) - 0.5) < 0.001);
});

test("konst: parses a literal to oklch", () => {
  const s = toOklch(konst("oklch(0 0 0 / 0.6)"));
  assert.ok(s.startsWith("oklch("));
  assert.ok(s.includes("0.6"));
});
